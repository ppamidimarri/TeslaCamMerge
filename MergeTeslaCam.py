...
ffmpeg_mid_full = '-filter_complex "[1:v]scale=w=1.2*iw:h=1.2*ih[top];[0:v]scale=w=0.4*iw:h=0.4*ih[right];[3:v]scale=w=0.4*iw:h=0.4*ih[back];[2:v]scale=w=0.4*iw:h=0.4*ih[left];[left][back][right]hstack=inputs=3[bottom];[top][bottom]vstack=inputs=2[full];[full]drawtext=text=\''
